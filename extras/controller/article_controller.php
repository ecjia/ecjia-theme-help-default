...
class article_controller
{
    ...
    public static function shop_notice()
    {
        $options = array(
            'article_type' => 'shop_help'
        );
        $shop_notice = RC_Api::api('article', 'article_list', $options);

        $article_id = intval($_GET['aid']);

        $cache_id = sprintf('%X', crc32($_SERVER['QUERY_STRING']));

        if (!ecjia_front::$controller->is_cached('article_shop_notice.dwt', $cache_id)) {
            $shop_notice = ecjia_api_manager::make()->api(ecjia_api_const::SHOP_NOTICE)->run();
            if (!is_ecjia_error($shop_notice)) {
                ecjia_front::$controller->assign('shop_notice', $shop_notice);
            }
            if(empty($article_id)) {
                $article_id = head($shop_notice)['id'];
            }

            $shop_notice_detail = RC_Api::api('article', 'article_info', array('id' => $article_id));
            dd($shop_notice_detail);
        ...
        ecjia_front::$controller->display('article_shop_notice.dwt');
    }
